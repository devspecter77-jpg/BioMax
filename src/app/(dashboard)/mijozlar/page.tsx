...
import { VILOYATLAR, viloyatTumanlari, toliqManzil } from '@/lib/hudud'
...
// Birlik enum'ini o'qiladigan qisqartmaga aylantiradi (DONA -> dona).
function birlikNomi(birlik: string): string {
  return birlik === 'DONA' ? 'dona' : birlik.toLowerCase()
}
...
export default function MijozlarPage() {
  ...
  // Ekranda ochilgan chek va yuborilayotgan sotuv id'si
  const [chekKorish, setChekKorish] = useState<MijozSotuv | null>(null)
  const [yuborilmoqda, setYuborilmoqda] = useState<string | null>(null)
  const [chekBalandligi, setChekBalandligi] = useState(420)

  useBodyScrollLock(modal || detailModal || !!chekKorish)
  const [tanlanganMijoz, setTanlanganMijoz] = useState<MijozDetail | null>(null)
  // Chek tili — do'kon kirill yozuvda chop etsa ham to'g'ri chiqishi uchun
  // (POS'dagi chek oynasi bilan bir xil tanlov).
  const [chekTili, setChekTili] = useState<'lotin' | 'kirill'>('lotin')

  // Chek HTML'i — chop etish ham, ekranda ko'rish ham AYNAN shu
  // natijani ishlatadi. POS'dagi bilan bir xil shablon, ya'ni qayta
  // chiqarilgan chek asl chekdan farq qilmaydi.
  function chekHtml(sotuv: MijozSotuv): string | null {
    if (!tanlanganMijoz) return null
    return buildChekHtml({
      data: {
        chekRaqami: sotuv.chekRaqami,
        sana: sotuv.sana,
        tarkiblar: sotuv.tarkiblar.map(t => ({
          tovar: { nomi: t.tovar.nomi },
          miqdor: t.miqdor,
        ...
        mijoz: { ism: tanlanganMijoz.ism },
        kassir: { telefon: sotuv.kassir?.telefon ?? null },
      },
      dokonInfo: tanlanganMijoz.dokon,
      til: chekTili,
    })
  }

  function chekniChopEt(sotuv: MijozSotuv) {
    const html = chekHtml(sotuv)
    if (html) chekChopEtish(html)
  }

  // Chekni mijozga Telegram orqali qayta yuborish. Sotuv paytida u
  // avtomatik bir marta ketadi — bu yerdan xohlagancha qayta jo'natiladi.
  async function chekniTelegramgaYubor(sotuv: MijozSotuv) {
    setYuborilmoqda(sotuv.id)
    try {
      const res = await fetch(`/api/sotuvlar/${sotuv.id}/chek-yuborish`, { method: 'POST' })
      const d = await res.json()
      if (!res.ok) { toast.error(d.xato || 'Yuborilmadi'); return }
      toast.success(`Chek ${d.mijoz || 'mijoz'}ga Telegram orqali yuborildi`)
    } catch {
  ...
  // Butun xarid tarixi — bitta uzun chek qog'ozida.
  function tarixniChopEt() {
    if (!tanlanganMijoz) return
    if (tanlanganMijoz.sotuvlar.length === 0) {
      toast.error("Bu mijozda xarid yo'q")
      return
    }
    const html = buildMijozTarixHtml({
      mijoz: {
        ism: tanlanganMijoz.ism,
        telefon: tanlanganMijoz.telefon,
        manzil: toliqManzil(tanlanganMijoz),
        maxsus_kod: tanlanganMijoz.maxsus_kod,
      },
      sotuvlar: tanlanganMijoz.sotuvlar.map(sv => ({
        chekRaqami: sv.chekRaqami,
        sana: sv.sana,
        chegirma: sv.chegirma,
        yakuniySumma: sv.yakuniySumma,
        tolovUsuli: sv.tolovUsuli,
        tarkiblar: sv.tarkiblar.map(t => ({
          tovar: { nomi: t.tovar.nomi },
          miqdor: t.miqdor,
          birlikNarxi: t.birlikNarxi,
          jami: t.jami,
        })),
        qaytarishlar: (sv.qaytarishlar || []).map(q => ({
          jamiSumma: q.jamiSumma,
          yaratilgan: q.yaratilgan,
          sabab: q.sabab,
          tarkiblar: q.tarkiblar.map(t => ({
            tovar: { nomi: t.tovar.nomi },
            miqdor: t.miqdor,
            birlikNarxi: t.birlikNarxi,
            jami: t.jami,
      ...
      jamiQarz: tanlanganMijoz.jami_qarz,
      dokonInfo: tanlanganMijoz.dokon,
      til: chekTili,
    })
    chekChopEtish(html)
  ...
  return (
    <div className="space-y-4">
            ...
            <table className="w-full table-fixed">
              <thead>
                <tr className="bg-gray-50 dark:bg-neutral-800 border-b border-gray-200 dark:border-neutral-800">
                  {kategoriya === 'top' && (
                    <th style={{ width: '44px' }} className="text-center text-gray-500 dark:text-gray-400 text-xs font-medium px-2 py-3 whitespace-nowrap">#</th>
                  )}
                  <th className="text-left text-gray-500 dark:text-gray-400 text-xs font-medium px-4 py-3">Ism</th>
                  <th style={{ width: '120px' }} className="text-left text-gray-500 dark:text-gray-400 text-xs font-medium px-4 py-3 hidden lg:table-cell whitespace-nowrap">Kod</th>
                  <th style={{ width: '180px' }} className="text-left text-gray-500 dark:text-gray-400 text-xs font-medium px-4 py-3 hidden sm:table-cell whitespace-nowrap">Telefon</th>
                  <th style={{ width: '160px' }} className="text-left text-gray-500 dark:text-gray-400 text-xs font-medium px-4 py-3 hidden md:table-cell whitespace-nowrap">Manzil</th>
                  <th style={{ width: '100px' }} className="text-center text-gray-500 dark:text-gray-400 text-xs font-medium px-4 py-3 whitespace-nowrap">Jami sotuv</th>
                  <th style={{ width: '140px' }} className="text-right text-gray-500 dark:text-gray-400 text-xs font-medium px-4 py-3 whitespace-nowrap">Qarz</th>
                  <th style={{ width: '60px' }} className="text-right text-gray-500 dark:text-gray-400 text-xs font-medium px-4 py-3 whitespace-nowrap">Amal</th>
                </tr>
              </thead>
              <tbody>
                {yuklanmoqda ? (
                  <tr><td colSpan={kategoriya === 'top' ? 8 : 7} className="text-center text-gray-500 dark:text-gray-400 py-12">Yuklanmoqda...</td></tr>
                ) : korsatiladiganMijozlar.length === 0 ? (
                  <tr><td colSpan={kategoriya === 'top' ? 8 : 7} className="py-12">
                    <div className="text-center">
                      <p className="text-sm text-gray-500 dark:text-gray-400">
                        {qidiruv ? 'Shu nom bo\'yicha mijoz topilmadi' : 'Hali mijoz qo\'shilmagan'}
                      </p>
                      {qidiruv ? (
                        <button
                          onClick={() => setQidiruv('')}
                  ...
                  <tr key={m.id} onClick={() => mijozOch(m.id)} className={`border-b border-gray-100 dark:border-neutral-800 hover:bg-gray-50 dark:hover:bg-neutral-800 transition cursor-pointer ${idx % 2 === 1 ? 'bg-gray-50/40 dark:bg-neutral-800/40' : ''}`}>
                    {kategoriya === 'top' && (
                      <td className="px-2 py-3 text-center">
                        <span className={`inline-flex items-center justify-center w-6 h-6 rounded-lg text-xs font-bold ${
                          idx === 0 ? 'bg-amber-100 text-amber-600' : idx === 1 ? 'bg-gray-200 text-gray-600' : idx === 2 ? 'bg-orange-100 text-orange-600' : 'text-gray-500 dark:text-gray-400'
                        }`}>{idx + 1}</span>
                      </td>
                    )}
                    ...
                    {/* Maxsus kod */}
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400 text-sm hidden lg:table-cell whitespace-nowrap">
                      {m.maxsus_kod ? (
      ...
      <div className={`grid grid-cols-1 gap-4 ${view === 'card' ? 'sm:grid-cols-2 lg:grid-cols-3' : 'sm:hidden'}`}>
          {yuklanmoqda ? (
            ...
            >
              ...
              {m.maxsus_kod && (
                <p className="mt-2 sm:mt-1.5 text-sm sm:text-xs font-mono text-gray-500 dark:text-gray-400 flex items-center gap-1">
              ...
              <div className="mt-4 sm:mt-3 pt-4 sm:pt-3 border-t border-gray-100 dark:border-neutral-800 grid grid-cols-2 gap-2">
                <div className="text-center">
                  <p className="text-gray-500 dark:text-gray-400 text-sm sm:text-xs">Jami sotuv</p>
                  <p className="text-gray-900 dark:text-gray-100 font-semibold text-lg sm:text-base">{m._count.sotuvlar} ta</p>
                </div>
                <div className="text-center">
                  <p className="text-gray-500 dark:text-gray-400 text-sm sm:text-xs">Qarz</p>
          ...
          <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-xl dark:shadow-none dark:border dark:border-neutral-800 w-full max-w-md">
            ...
            <form onSubmit={saqlash} className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-gray-700 dark:text-gray-300 text-sm mb-1 block font-medium">Viloyat</label>
                  <Combobox
                    options={VILOYATLAR.map(v => ({ value: v, label: v }))}
                    value={form.viloyat}
                    onChange={v => setForm(f => ({
                      ...f,
                      viloyat: v,
                      tuman: viloyatTumanlari(v).includes(f.tuman) ? f.tuman : '',
                    }))}
                ...
                <div>
                  <label className="text-gray-700 dark:text-gray-300 text-sm mb-1 block font-medium">Tuman / shahar</label>
                  <Combobox
                    options={viloyatTumanlari(form.viloyat).map(t => ({ value: t, label: t }))}
                    value={form.tuman}
                    onChange={v => setForm(f => ({ ...f, tuman: v }))}
                    placeholder={form.viloyat ? 'Tanlang yoki yozing' : 'Yozing'}
                    searchPlaceholder="Tuman nomi..."
                    emptyMessage="Ro'yxat yo'q — qo'lda yozing"
                    allowCustom
                  />
                </div>
              </div>

              {/* Manzil — ko'cha/uy darajasidagi tafsilot */}
              <div>
                <label className="text-gray-700 dark:text-gray-300 text-sm mb-1 block font-medium">
                  Manzil <span className="text-gray-400 font-normal">(ko&apos;cha, uy)</span>
                </label>
                <input type="text"
                  value={form.manzil}
      ...
      {/* ── Chekni ochib ko'rish ──
          Chop etiladigan HTML'ning AYNAN o'zi iframe ichida
          ko'rsatiladi: ekranda ko'rgan narsa printerdan chiqadi. */}
      {chekKorish && (
        <div
          className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-[60] sm:p-4"
          onClick={() => setChekKorish(null)}
        >
          <div
            onClick={e => e.stopPropagation()}
            className="w-full sm:max-w-sm flex flex-col max-h-[88dvh] bg-white dark:bg-neutral-900 rounded-t-2xl sm:rounded-2xl shadow-xl overflow-hidden"
          >
            <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-neutral-800">
              <div className="min-w-0">
                <p className="text-gray-900 dark:text-gray-100 font-semibold text-sm truncate">
                  {chekKorish.chekRaqami}
                </p>
                <p className="text-gray-500 dark:text-gray-400 text-xs">
                  {formatSanaVaVaqt(chekKorish.sana)}
                </p>
              </div>
              <button
                onClick={() => setChekKorish(null)}
                className="p-1.5 -mr-1.5 text-gray-400 hover:bg-gray-100 dark:hover:bg-neutral-800 rounded-lg transition shrink-0"
                aria-label="Yopish"
              >
                <X size={18} />
              </button>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto bg-gray-100 dark:bg-neutral-950 p-3">
              <iframe
                title={`Chek ${chekKorish.chekRaqami}`}
                srcDoc={chekHtml(chekKorish) ?? ''}
                className="w-full bg-white rounded-lg border-0 block"
                style={{ height: chekBalandligi }}
                // Chek uzunligi mahsulot soniga qarab har xil — yuklangach
                // haqiqiy balandligini o'lchab olamiz, aks holda qisqa chek
                // ostida bo'sh oq maydon qolardi.
                onLoad={e => {
                  const hujjat = e.currentTarget.contentDocument
                  const h = hujjat?.documentElement?.scrollHeight
                  if (h && h > 40) setChekBalandligi(h + 8)
                }}
              />
            </div>

            <div className="shrink-0 flex gap-2 p-3 border-t border-gray-200 dark:border-neutral-800 pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
              <button
                onClick={() => chekniChopEt(chekKorish)}
                className="flex-1 py-2.5 border border-gray-300 dark:border-neutral-700 text-gray-600 dark:text-gray-400 rounded-xl hover:bg-gray-50 dark:hover:bg-neutral-800 transition font-medium text-sm flex items-center justify-center gap-2"
              >
                <Printer size={15} /> Chop etish
              </button>
              <button
                onClick={() => chekniTelegramgaYubor(chekKorish)}
                disabled={yuborilmoqda === chekKorish.id}
                className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-60 text-white rounded-xl font-medium transition text-sm flex items-center justify-center gap-2"
              >
                {yuborilmoqda === chekKorish.id
          ...
          <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-xl dark:border dark:border-neutral-800 w-full max-w-lg max-h-[85vh] overflow-y-auto">
            <div className="p-5 border-b border-gray-200 dark:border-neutral-800 flex items-center justify-between sticky top-0 bg-white dark:bg-neutral-900 z-10">
              ...
              <div className="flex items-center gap-1 shrink-0">
                {tanlanganMijoz && tanlanganMijoz.sotuvlar.length > 0 && (
                  <>
                    <button
                      onClick={() => setChekTili(chekTili === 'lotin' ? 'kirill' : 'lotin')}
                      className="flex items-center gap-1 px-2 py-1.5 text-xs text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-neutral-800 rounded-lg transition"
                      title="Chek tilini almashtirish"
                    >
                      <Languages size={14} />
                      {chekTili === 'lotin' ? 'Lotin' : 'Kirill'}
                    </button>
                    <button
                      onClick={tarixniChopEt}
                      className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium bg-primary-light dark:bg-primary/10 text-primary hover:bg-primary/20 rounded-lg transition"
                      title="Butun xarid tarixini chek qog'oziga chop etish"
                    >
                      <Printer size={14} />
                      Tarixni chop etish
                    </button>
                ...
                <button onClick={() => setDetailModal(false)} className="p-1.5 text-gray-500 dark:text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-neutral-800 rounded-lg transition">
                  <X size={18} />
                </button>
              </div>
            </div>
            ...
            {detailYuklanmoqda ? (
              ...
              <p className="p-8 text-center text-gray-500 dark:text-gray-400">Ma&apos;lumot topilmadi</p>
            ) : (
              <div className="p-5 space-y-4">
                <div className="flex flex-wrap gap-2 text-sm">
                  ...
                  {tanlanganMijoz.telefon2 && (
                    <a href={`tel:${tanlanganMijoz.telefon2.replace(/\s/g, '')}`} className="flex items-center gap-1 text-blue-500 hover:text-blue-600 bg-blue-50 dark:bg-blue-950/30 px-2.5 py-1 rounded-lg">
                      <Phone size={12} />{formatPhone(tanlanganMijoz.telefon2)}
                  ...
                  {Number(tanlanganMijoz.ballBalans || 0) > 0 && (
                    <span className="flex items-center gap-1 text-amber-700 dark:text-amber-500 bg-amber-50 dark:bg-amber-950/30 px-2.5 py-1 rounded-lg font-medium">
                      {formatBall(tanlanganMijoz.ballBalans || 0)} ball
                    </span>
                  )}
                  {Number(tanlanganMijoz.keshbekBalans || 0) > 0 && (
                    <span className="flex items-center gap-1 text-amber-700 dark:text-amber-500 bg-amber-50 dark:bg-amber-950/30 px-2.5 py-1 rounded-lg font-medium">
                      Keshbek: {formatSum(tanlanganMijoz.keshbekBalans || 0)}
                    </span>
                  )}
                ...
                <div>
                  ...
                  {tanlanganMijoz.sotuvlar.length === 0 ? (
                    <p className="text-gray-500 dark:text-gray-400 text-sm text-center py-6">Hali xarid qilmagan</p>
                  ) : (
                        ...
                        return tarix.map(y => y.turi === 'sotuv' ? (
                          <div key={y.sotuv.id} className="border border-gray-200 dark:border-neutral-700 rounded-xl p-3">
                            <div className="flex items-center justify-between gap-2">
                              <span className="text-xs font-mono text-gray-500 dark:text-gray-400 truncate">{y.sotuv.chekRaqami}</span>
                              <div className="flex items-center gap-1 shrink-0">
                                <span className="text-green-600 font-semibold text-sm mr-1">{formatSum(y.sotuv.yakuniySumma)}</span>
                                <button
                                  onClick={() => { setChekBalandligi(420); setChekKorish(y.sotuv) }}
                                  className="p-1.5 text-gray-400 hover:text-primary hover:bg-primary-light dark:hover:bg-primary/10 rounded-md transition"
                                  title="Chekni ochib ko'rish"
                                >
                                  <Receipt size={14} />
                                </button>
                                <button
                                  onClick={() => chekniChopEt(y.sotuv)}
                                  className="p-1.5 text-gray-400 hover:text-primary hover:bg-primary-light dark:hover:bg-primary/10 rounded-md transition"
                                  title="Chop etish"
                                >
                                  <Printer size={14} />
                                ...
                                <button
                                  onClick={() => chekniTelegramgaYubor(y.sotuv)}
                                  disabled={yuborilmoqda === y.sotuv.id}
                                  className="p-1.5 text-gray-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-950/30 rounded-md transition disabled:opacity-50"
                                  title="Mijozga Telegram orqali yuborish"
                                >
                                  {yuborilmoqda === y.sotuv.id
                              ...
                              {y.sotuv.tarkiblar.map(t => (
                                <div key={t.id} className="flex items-center justify-between text-xs text-gray-600 dark:text-gray-400">
                                  <span className="truncate">
                                    {t.tovar.nomi}
                                    <span className="text-gray-500 dark:text-gray-400"> — {t.miqdor} {birlikNomi(t.tovar.birlik)} × {formatSum(t.birlikNarxi)}</span>
                                  </span>
